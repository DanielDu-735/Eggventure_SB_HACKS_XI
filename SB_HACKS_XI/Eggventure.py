...
# Load Images
obstacle_image = pygame.image.load('obstacle.png')  # Ensure this path is correct
obstacle_image = pygame.transform.scale(obstacle_image, (50, 50))  # Scale obstacle image

triangle_image = pygame.image.load('triangle.png')  # Ensure this path is correct
triangle_image = pygame.transform.scale(triangle_image, (30, 30))  # Scale triangle image
...
class Egg:
    def __init__(self, color):
        ...
        self.width = EGG_RADIUS * 2  # Egg width will be double the radius
        self.height = EGG_RADIUS * 2.5  # Make the egg slightly taller than it is wide (egg-like shape)
    ...
    def draw(self, screen):
        ...
        # Draw the entire oval shape of the egg
        pygame.draw.ellipse(
            screen, color,
...
class Obstacle:
    ...
    def draw(self, screen):
        screen.blit(obstacle_image, (self.x + 30 // 2 - obstacle_image.get_width() // 2, 
                                     self.y + 30 // 2 - obstacle_image.get_height() // 2))
...
class Triangle:
    ...
    def draw(self, screen):
        screen.blit(triangle_image, (self.x + self.size // 2 - triangle_image.get_width() // 2, 
                                     self.y + self.size // 2 - triangle_image.get_height() // 2))
...
def title_screen():
    ...
    # Title text
    title_text = title_font.render("Welcome to the Eggventure!", True, (0, 0, 0))
...
def start_page():
    ...
    # Game Rules text
    rules_text_1 = font.render("To get a chick, you need to collect 10 nutrition packs", True, (0, 0, 0))
    rules_text_2 = font.render("and protect your egg.", True, (0, 0, 0))
    rules_text_3 = font.render("Each time you hit a bomb, your egg's health will decrease by 25%.", True, (0, 0, 0))
    rules_text_4 = font.render("Every 5 seconds, your egg will recover 25% health, up to 100%.", True, (0, 0, 0))
    rules_text_5 = font.render("Good luck!", True, (0, 0, 0))
    
    screen.blit(rules_text_1, (WIDTH // 2 - rules_text_1.get_width() // 2, HEIGHT // 2 + 90))
    screen.blit(rules_text_2, (WIDTH // 2 - rules_text_2.get_width() // 2, HEIGHT // 2 + 120))
    screen.blit(rules_text_3, (WIDTH // 2 - rules_text_3.get_width() // 2, HEIGHT // 2 + 150))
    screen.blit(rules_text_4, (WIDTH // 2 - rules_text_4.get_width() // 2, HEIGHT // 2 + 180))
    screen.blit(rules_text_5, (WIDTH // 2 - rules_text_5.get_width() // 2, HEIGHT // 2 + 210))
...
def game():
    ...
    while not game_over:
        ...
        for obstacle in obstacles:
            if (egg.x - obstacle.x)**2 + (egg.y - obstacle.y)**2 < (EGG_RADIUS + 30 // 2)**2:
...
def end_page(score, time_elapsed):
    ...
    background_image = pygame.image.load('background_end_page.png')  # Change this path to your image file
    background_image = pygame.transform.scale(background_image, (WIDTH, HEIGHT))  # Scale it to fit the screen
    
    # Draw the background image
    screen.blit(background_image, (0, 0))  # Position it at the top-left corner
